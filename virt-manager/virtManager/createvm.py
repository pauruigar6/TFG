...
import datetime                           
import libvirt                            
from .template_utils import TEMPLATES_ROOT 
...
class vmmCreateVM(vmmGObjectUI):
    ...
    def __init__(self):
        ...
        # Callbacks mínimos que faltaban para el mediacombo (ISO)
        def _iso_changed_cb(mediacombo, entry):
            # Cuando cambie el texto en el mediacombo de ISO
            self._detectable_media_widget_changed(entry)

        def _iso_activated_cb(mediacombo, entry):
            # Cuando el usuario pulsa Enter en el mediacombo de ISO
        ...
        # -------------------------------------------------------
        #  Conexión del mediacombo (ISO local)
        # -------------------------------------------------------
    ...
    def _populate_summary(self):
        ...
        # Si guest.memory es None (por ejemplo, al importar desde plantilla),
        # dejamos el texto vacío en lugar de convertir a int(None)
        if guest.memory is not None:
            mem = _pretty_memory(int(guest.memory))
        else:
            mem = ""

        # Lo mismo para vcpus
        if guest.vcpus is not None:
    ...
    def _validate_install_page(self):
        ...
        # 2) Si no estamos en “template”, seguimos con la lógica normal:
        instmethod = self._get_config_install_page()
        installer = None
        location = None
        extra = None
    ...
    def _validate_mem_page(self):
        ...
        # Si “Importar desde plantilla” está activo, no validamos memoria
        if self.widget("method-template").get_active():
            return True
